...
if command -v python > /dev/null 2>&1; then
	...
	echo -e "\e[31m- Python is not installed or not found in PATH.\e[0m" >&2
	exit_code=1
fi
...
if command -v uv > /dev/null 2>&1; then
	echo -e "\e[32m- uv $(command_version uv --version) installed.\e[0m"
else
	echo -e "\e[31m- uv is not installed or not found in PATH.\e[0m" >&2
	exit_code=1
fi

if command -v pnpm exec oxfmt > /dev/null 2>&1; then
	echo -e "\e[32m- Oxfmt $(command_version pnpm exec oxfmt --version) installed.\e[0m"
else
...
if command -v rendercv > /dev/null 2>&1; then
	echo -e "\e[32m- RenderCV $(command_version rendercv --version) installed.\e[0m"
else
	echo -e "\e[31m- RenderCV is not installed or not found in PATH.\e[0m" >&2
	exit_code=1
fi
